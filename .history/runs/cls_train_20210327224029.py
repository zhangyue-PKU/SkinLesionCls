...
if __name__ == '__main__':
    ...
    import os
    #os.environ['PYTHONPATH'] = "$/home/zhangyue/Documents/lesion-segmentation-classification"
    ...
    for k_fold in range(num_folds):
        ...
        run_name = 'task3_' + backbone_name + \
            '_k' + str(k_fold) + '_v' + version
        ...
        num_classes = y_train.shape[1] #num of classes

        callbacks = config_cls_callbacks(run_name) #????

        model = backbone(backbone_name, **backbone_options).classification_model( #choose a backbone by name and call its classification function
        ...
        if use_data_aug:
            ...
            log_variable(var_name='width_shift_range',
                         var_value=width_shift_range)
            log_variable(var_name='height_shift_range',
                         var_value=height_shift_range)
